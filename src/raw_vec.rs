...
use allocator_api::alloc::{Alloc, Layout, Global, handle_alloc_error};
use allocator_api::collections::CollectionAllocErr::{self, *};
use allocator_api::boxed::Box;

use crate::atomic_ptr::AtomicUnique;
...
/// In particular:
///
/// * Produces AtomicUnique::empty() on zero-sized types
/// * Produces AtomicUnique::empty() on zero-length allocations
/// * Catches all overflows in capacity computations (promotes them to "capacity overflow" panics)
/// * Guards against 32-bit systems allocating more than isize::MAX bytes
/// * Guards against overflowing your length
/// * Aborts on OOM or calls handle_alloc_error as applicable
/// * Avoids freeing AtomicUnique::empty()
/// * Contains a ptr::AtomicUnique and thus endows the user with all related benefits
///
...
impl<T, A: Alloc> RawVec<T, A> {
    ...
    pub const fn new_in(a: A) -> Self {
        ...
        // AtomicUnique::empty() doubles as "unallocated" and "zero-sized allocation"
        RawVec {
            ptr: AtomicUnique::empty(),
...
impl<T, A: Alloc> RawVec<T, A> {
    ...
    pub unsafe fn from_raw_parts_in(ptr: *mut T, cap: usize, a: A) -> Self {
        RawVec {
            ptr: AtomicUnique::new_unchecked(ptr),
            cap,
            a,
        }
    }
...
impl<T, A: Alloc> RawVec<T, A> {
    ...
    /// AtomicUnique::empty() if `cap = 0` or T is zero-sized. In the former case, you must
    /// be careful.
        ...
        unsafe {
            ...
            let (new_cap, uniq) = match self.current_layout() {
                Some(cur) => {
                    ...
                    let ptr_res = self.a.realloc(Into::<NonNull<_>>::into(self.ptr).cast(),
                                                 cur,
                                                 new_size);
                    match ptr_res {
    ...
    pub fn double_in_place(&mut self) -> bool {
        unsafe {
            ...
            let new_size = new_cap * elem_size;
            alloc_guard(new_size).unwrap_or_else(|_| capacity_overflow());
            match self.a.grow_in_place(Into::<NonNull<_>>::into(self.ptr).cast(), old_layout, new_size) {
                Ok(_) => {
    ...
    pub fn reserve_in_place(&mut self, used_cap: usize, needed_extra_cap: usize) -> bool {
        unsafe {
            ...
            match self.a.grow_in_place(
                Into::<NonNull<_>>::into(self.ptr).cast(), old_layout, new_layout.size(),
            ) {
                Ok(_) => {
                    self.cap = new_cap;
                    true
    ...
    pub fn shrink_to_fit(&mut self, amount: usize) {
        ...
        if amount == 0 {
            ...
            unsafe {
                ...
                match self.a.realloc(Into::<NonNull<_>>::into(self.ptr).cast(),
                                     old_layout,
                                     new_size) {
...
impl<T, A: Alloc> RawVec<T, A> {
    fn reserve_internal(
        ...
        unsafe {
            use allocator_api::alloc::AllocErr;
            ...
            let res = match self.current_layout() {
                Some(layout) => {
                    ...
                    self.a.realloc(Into::<NonNull<_>>::into(self.ptr).cast(), layout, new_layout.size())
                }
    ...
    pub unsafe fn dealloc_buffer(&mut self) {
        ...
        if elem_size != 0 {
            if let Some(layout) = self.current_layout() {
                self.a.dealloc(Into::<NonNull<_>>::into(self.ptr).cast(), layout);
            }
        }
    }
}
...
fn alloc_guard(alloc_size: usize) -> Result<(), CollectionAllocErr> {
    if mem::size_of::<usize>() < 8 && alloc_size > std::isize::MAX as usize {
